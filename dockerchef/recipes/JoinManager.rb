...
Chef::Log.info("Got managerkey item for #{managerKey}")

manageraddrcommand = ""
workercommand << "aws dynamodb get-item --table-name MySwarmTable  --key '{\"TokenKey\": {\"S\": \"#{managerAddrKey}\"}}'"
...
Chef::Log.info("Got manager address for #{managerAddrKey}")

swarmJoinCommand = ""
swarmJoinCommand << "docker swarm --token #{token} #{managerAddr}"
Chef::Log.info("Manager Joining Swarm")
